...
function tpuTemperature(callback) {

  // for mon in /sys/class/apex/apex_*; do echo $(ls "$mon"/temp)___$(cat "$mon"/temp); done;
  return new Promise((resolve) => {
    process.nextTick(() => {
      let result = {
        main: null,
      ...
      if (_linux) {
        const cmd = 'for mon in /sys/class/apex/apex_*; do echo $(ls "$mon")___$(cat "$mon"/temp); done;';
        try {
          exec(cmd, function (error, stdout) {
            ...
            lines.forEach(line => {
              ...
              const label = parts[0].split("apex_")[1];
              const value = parts.length > 0 ? parts[1] : '0';
...
exports.tpuTemperature = tpuTemperature;
